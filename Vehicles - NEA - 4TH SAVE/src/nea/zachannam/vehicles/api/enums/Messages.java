package nea.zachannam.vehicles.api.enums;
...
import nea.zachannam.vehicles.api.database.Config;
...
public enum Messages {
	...
	@Getter
	private static HashMap<Messages, String> customMessages = new HashMap<Messages,String>();
	...
	public static void reload() {
		...
		for(Messages message : Messages.values()) {
			...
			if(configFile.isString("messages." + message.toString())) {
				String messageValue = configFile.getString("messages." + message.toString());
				getCustomMessages().put(message, ChatColor.translateAlternateColorCodes('&', messageValue));
			} else {
				configFile.set("messages."+message.toString(), message.getRawMessage().replace('§', '&'));
			}
	...
	public void sendCommandSender(CommandSender paramCommandSender, ImmutableMap<String, String> paramFormat) {
		...
		for(String format : paramFormat.keySet()) {
			...
			messagep = messagep.replaceAll(format.toString(), formatTo.toString());
		}
